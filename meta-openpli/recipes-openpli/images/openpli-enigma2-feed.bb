...
OPTIONAL_WIFI_PACKAGES = "\
	...
	${@ 'kernel-module-8852au'     if ("${KERNEL_VERSION}" and bb.utils.vercmp_string("${KERNEL_VERSION}", '3.10') >= 0) else '' } \
...
OPTIONAL_PACKAGES += " \
	${@ 'wireguard-tools' if ("${KERNEL_VERSION}" and bb.utils.vercmp_string("${KERNEL_VERSION}", '3.10') >= 0) else '' } \
	astra-sm \
	autofs \
	autossh \
	ccid \
...
OPTIONAL_ENIGMA2_PACKAGES = " \
	...
	enigma2-plugin-skins-pli-fullnight-hd \
